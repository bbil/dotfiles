...
/* -----------------------------
   ...
   Now pure white, dimmer, and semi-transparent
   ----------------------------- */
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    /* -----------------------------
       MODIFIED: White, dimmer, semi-transparent trail
       ----------------------------- */
    vec4 whiteTrail = vec4(vec3(1.0) * TRAIL_BRIGHTNESS, TRAIL_COLOR.a);

    vec4 trail = mix(whiteTrail, fragColor, 1. - smoothstep(0., sdfTrail + mod, 0.007));
    trail = mix(whiteTrail, trail, 1. - smoothstep(0., sdfTrail + mod, 0.006));
    trail = mix(trail, whiteTrail, step(sdfTrail + mod, 0.));
    trail = mix(whiteTrail, trail, 1. - smoothstep(0., sdfCurrentCursor + .002, 0.004));
    trail = mix(whiteTrail, trail, 1. - smoothstep(0., sdfCurrentCursor + .002, 0.004));
